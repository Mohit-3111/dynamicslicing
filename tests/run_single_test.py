...
def correct_output(expected: str, actual: str) -> bool:
    ...
    expected_trimmed = "\n".join([l.rstrip() for l in expected_lines if (l.strip() != "" and not l.strip().startswith("#"))])
    actual_trimmed = "\n".join([l.rstrip() for l in actual_lines if (l.strip() != "" and not l.strip().startswith("#"))])
...
def test_runner(directory_pair: Tuple[str, str], capsys):
    ...
    elif module_prefix.startswith("milestone3") or module_prefix.startswith("hidden"):
    ...
    selected_hooks = get_hooks_from_analysis([f"{module_name}.{ac[0]};source_path={program_file}" for ac in analysis_classes])
